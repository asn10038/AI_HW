...
class HMM(object):
    ...
    def propagate(self, belief):
        ...
        # doesn't seem to keep sum to 1
        new_belief = [0]*len(belief)
        for i in range(len(belief)):
            for j in range(len(belief)):
                new_belief[i] += belief[j]*self.tprob[j][i]
        belief = new_belief
        return belief
    ...
    def filter(self, observations):
        ...
        for x in range(len(observations)):
            if x != 0:
                belief = self.propagate(belief)
            belief = self.observe(belief, observations[x])
...
if __name__ == '__main__':
    ...
    belief = new_hmm.initial
    print(belief)
    new_belief = new_hmm.propagate(belief)
    print(new_belief)
